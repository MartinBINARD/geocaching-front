import { Link } from 'react-router-dom';

// import of picture and icons for the homepage
import logo from '../../assets/logo/compass.png';
import Footer from '../../components/Footer/Footer';

function Homepage() {
  return (
    <>
      <div className="lg:h-screen w-screen absolute">
        {/* <img
          className="absolute bottom-0 object-fill w-full "
          src={Bg}
          alt="background illustration"
        /> */}
      </div>
      <div className="flex flex-col justify-center z-10 items-center lg:flex-row m-auto lg:gap-20">
        {/* <img
          className="max-w-xs m-auto z-10 lg:m-5 shrink"
          src={koala}
          alt="illustration d'accueil"
        /> */}
        <section className="flex flex-col gap-10 w-3/4 lg:w-1/3 text-primary">
          <h3 className="font-bold text-4xl">
            Découvre le monde sous un
            <span className="text-accent">nouvel angle</span>
          </h3>
          <p className="text-xl">
            Rejoins la communauté des explorateurs de
            <span className="font-bold italic">CachTrek</span>, parcours des
            endroits étonnants et apprend tout en t&apos;amusant !
          </p>
          <Link
            className="flex w-max items-center gap-2 border rounded-md btn btn-primary btn-lg normal-case text-xl font-medium text-white"
            to="/presentation"
          >
            <img className="h-10" src={logo} alt="Logo CacheTrek" />
            Découvrir
          </Link>
        </section>
      </div>
      <Footer />
    </>
  );
}

export default Homepage;
